    ...
    <div className="flex item justify-between py-5 font-medium">
      ...
      <ul className="hidden sm:flex gap-5 text-lg text-gray-700">
        <NavLink to="/" className="flex flex-col items-center gap-1">
          <p>Home</p>
          <hr className="w-2/4 border-t border-gray-700 h-[1.5px] hidden" />
        </NavLink>
        <NavLink to="/collections" className="flex flex-col items-center gap-1">
          <p>Collection</p>
          <hr className="w-2/4 border none h-[1.5px] b-grey-700 hidden" />
        </NavLink>
        ...
        <div className="group relative">
          ...
          onClick={() => {
            if (!token) {
              ...
              setDropdownOpen(!dropdownOpen); // toggle on click
            }
          }}
     ...
     <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
      <div className='flex flex-col text-gray-600'>
        <div onClick={()=>setVisible(false)} className='flex items-center gap-4 p-3 cursor-pointer'>
          <img src={assets.dropdown_icon} className="h-4 rotate-180" alt="Dropwdown icon" />
          <p>back</p>
        </div>
